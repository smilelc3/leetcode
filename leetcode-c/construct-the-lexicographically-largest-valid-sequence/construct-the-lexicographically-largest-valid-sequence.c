...
bool dfsSolver(int *ans, int idx, int maxN, bool *visit) {
    if (idx == 2 * maxN - 1) {
        return true;
    }
    if (ans[idx] != 0) {       // 当前位置已经有数填充
        return dfsSolver(ans, idx + 1, maxN, visit);
    }
    for (int n = maxN; n >= 1; --n) {
        ...
        visit[n] = true;
        if (dfsSolver(ans, idx + 1, maxN, visit) == true) {
            return true;
        }
        ans[idx] = 0;
        if (n != 1) {
...
int *constructDistancedSequenceDFSMethod(int n, int *returnSize) {
    *returnSize = 2 * n - 1;
    int *ans = calloc(1, sizeof(int) * *returnSize);
    bool *visit = calloc(1, sizeof(bool) * (n + 1));
    dfsSolver(ans, 0, n, visit);   // 必然有解
    free(visit);
    return ans;
}


int *constructDistancedSequenceTableMethod(int n, int *returnSize) {
    int allAns[][20 * 2 - 1] = {
            {1},
            {2,  1,  2},
            {3,  1,  2,  3,  2},
    ...
    *returnSize = 2 * n - 1;
    ...
    memcpy(ans, allAns[n - 1], sizeof(int) * *returnSize);
    return ans;
}
...
int *constructDistancedSequence(int n, int *returnSize) {
    ...
    // DFS
    // return constructDistancedSequenceDFSMethod(n, returnSize);

    // 打表
    return constructDistancedSequenceTableMethod(n, returnSize);
}