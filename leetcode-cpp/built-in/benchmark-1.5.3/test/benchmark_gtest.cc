...
namespace benchmark {
    namespace internal {
        ...
        AddRange(&dst,
        static_cast<int64_t>(1), static_cast<int64_t>(2), 2);
        EXPECT_THAT(dst, testing::ElementsAre(1, 2)
        );
    }
...
std::vector<int64_t> dst;
AddRange(&dst,
static_cast<int64_t>(5), static_cast<int64_t>(15), 2);
EXPECT_THAT(dst, testing::ElementsAre(5, 8, 15)
);
}
...
AddRange(&dst, int8_t{1}, std::numeric_limits<int8_t>::max(),
8);
...
std::vector<int64_t> dst;
AddRange(&dst, int64_t{1}, std::numeric_limits<int64_t>::max(),
1024);
EXPECT_THAT(
