//
// Created by smile on 2022/4/11.
//
#include <gtest/gtest.h>
#include "h-index-ii.cpp"


namespace {
    TEST(hIndex2Test, Example1) {      // NOLINT
        std::vector<int> citations = {0, 1, 3, 5, 6};
        auto ans = Solution::hIndex(citations);
        decltype(ans) rightAns = 3;
        ASSERT_EQ(ans, rightAns);
    }

    TEST(hIndex2Test, Example2) {      // NOLINT
        std::vector<int> citations = {1, 2, 100};
        auto ans = Solution::hIndex(citations);
        decltype(ans) rightAns = 2;
        ASSERT_EQ(ans, rightAns);
    }

    TEST(hIndex2Test, Test0) {      // NOLINT
        std::vector<int> citations = {1};
        auto ans = Solution::hIndex(citations);
        decltype(ans) rightAns = 1;
        ASSERT_EQ(ans, rightAns);
    }

    TEST(hIndex2Test, Test1) {      // NOLINT
        std::vector<int> citations = {11, 15};
        auto ans = Solution::hIndex(citations);
        decltype(ans) rightAns = 2;
        ASSERT_EQ(ans, rightAns);
    }

    TEST(hIndex2Test, Test2) {      // NOLINT
        std::vector<int> citations = {0, 0};
        auto ans = Solution::hIndex(citations);
        decltype(ans) rightAns = 0;
        ASSERT_EQ(ans, rightAns);
    }
}

int main(int argc, char *argv[]) {
    testing::InitGoogleTest(&argc, argv);
    return RUN_ALL_TESTS();
}
